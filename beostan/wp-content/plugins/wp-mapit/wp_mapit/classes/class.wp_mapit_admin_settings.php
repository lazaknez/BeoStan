	...
	if( ! class_exists( 'wp_mapit_admin_settings' ) ) {
		...
		class wp_mapit_admin_settings
		{
			...
			public static function init()
			{
				...
				/* Change menu order */
				add_filter( 'custom_menu_order', __CLASS__ . '::custom_menu_order' );
			}
			...
			public static function admin_menu() {
				...
				add_submenu_page("wp_mapit", __('Settings', WP_MAPIT_TEXTDOMAIN), __('Settings', WP_MAPIT_TEXTDOMAIN), 'manage_options', 'wp_mapit', __CLASS__ . '::wp_mapit_settings');
			}
			...
			public static function custom_menu_order($menu_ord) {
				...
				if( isset( $submenu['wp_mapit'] ) && isset( $submenu['wp_mapit'][0] ) ) {
					$submenu['wp_mapit'][99] = $submenu['wp_mapit'][0];
									...
									<div class="wp-mapit-row">
										...
										<div class="wp-mapit-row">
											...
											<?php
												...
												$arrExcludePostTypes = apply_filters('wp_mapit_exclude_post_types', array( 'wp_mapit_map', 'attachment' ));
											...
											<?php
														foreach ( $arrPostTypes as $postType ) {
															if( ! in_array( $postType, $arrExcludePostTypes ) ) {
											?>
